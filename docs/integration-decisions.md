...
최신 갱신: 2026-09-26. Live OIDC/서버 세션·관리자 제어 claim을 통한 자동 메일 연결·Mailcow API 앱 비밀번호 발급/암호화 보관·IMAP/SMTP 및 제한된 계정 관리 코드를 추가했습니다. 격리 테스트 통과, 운영 연결 검증은 미실시입니다. **최신 실행 범위는 [Live 설치](live-installation.md), 실제 계약은 [Live API](live-api.md)를 기준으로 합니다.** 아래는 2026-09-24 초기 조사/설계 기록입니다.
